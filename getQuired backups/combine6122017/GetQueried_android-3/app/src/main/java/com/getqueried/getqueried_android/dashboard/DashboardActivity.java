...
public class DashboardActivity extends NetworkActivity {
  ...
  /*  @Bind(R.id.tabs_dashboard)
    ...
    private void getPendingQueries() {
        ...
        getString(
                Constants.URL.getQueryIdList, response -> {
                    Crashlytics.log(Log.DEBUG, TAG, "Dashboard id222 : " + response);
                    getQueryMetadata(response);
                }, null);

    }

    private void getQueryMetadata(String queryId) {
        Crashlytics.log(Log.DEBUG, TAG, "get222QueryMetadata called !!");
        for(int i=0;i<queryId.length();i++) {

            try {
                jsonObject = new JSONArray(queryId);
            } catch (JSONException e) {
                e.printStackTrace();
            }
            try {
                postStringUrlEncoded(Constants.URL.getQueryMetadata, "[\"" + jsonObject.get(i) + "\"]", response -> {
                    //  Crashlytics.log(Log.DEBUG, TAG, "Response22 Query MetadataList : " + response);
                    try {
